...
class MoveNet(private val interpreter: Interpreter, private var gpuDelegate: GpuDelegate?) :
    ...
    fun detectPose(bitmap: Bitmap): Person {
        ...
        val keyPoints = persons.firstOrNull()?.keyPoints ?: listOf() // KeyPoint 리스트 가져오기
        val score = persons.firstOrNull()?.score ?: 0f  // 점수 가져오기

        // Person 객체 생성 시 KeyPoint 리스트와 score를 명시적으로 전달
        return Person(keyPoints = keyPoints, score = score)
    }
